##################################################################################################################################
## PROGRAM: TSS GLM cv with parallel.R
## SPECIFICATIONS: Develops ensemble GLM
## for predicting TSS
#####################################################################################################################################
...
perf <- foreach(i=1:nrow(modCBnew), .combine=rbind) %dopar%{
  ...
  ##Train glm
  TSSglm<-glm(TSS ~ ., data=CBtrain,family=Gamma(link="log"))
  
  ##Save the model 
  save(TSSglm, file = paste0("D:/Publication/TSS/GLM models/glm",i,".RData"))
  
  ##Adjoin the information given
  c(AIC(TSSglm), ##Akaike Information Criterion
    BIC(TSSglm), ##Bayesian Information Criterion
    as.vector(predict(TSSglm,CBtest))) ##predicted on test dataset
}
stopImplicitCluster()
