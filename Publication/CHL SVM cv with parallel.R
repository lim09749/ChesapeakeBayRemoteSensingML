##################################################################################################################################
## PROGRAM: CHL SVM cv with parallel.R
## SPECIFICATIONS: Develops ensemble support vector machines for predicting chlorophyll
#####################################################################################################################################
...
perf <- foreach(i=1:nrow(modCBnew), .combine=rbind) %dopar%{
  ...
  ##Train svm
  tuneResult <- tune(svm,chl~ .,
                     data=StdLogCBtrain,
                     ranges = list(epsilon = seq(0.1,0.5,0.1),
                                   cost = 2^(0:3)),
                     type="eps-regression",kernel="linear")
  chlsvm <- tuneResult$best.model
  
  
  ##Save the model 
  save(chlsvm, file = paste0("D:/Publication/CHL/SVM models/svm",i,".RData"))
  
  ##Adjoin the information given
  c(chlsvm$tot.nSV, ##Number of support vectors
    as.vector(10^(
      predict(chlsvm, StdLogCBtest)*log_scale[1]+log_center[1]))) ##predicted on test dataset
}
stopImplicitCluster()
