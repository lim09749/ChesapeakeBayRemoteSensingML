##################################################################################################################################
## PROGRAM: CHL RVM cv with parallel.R
## SPECIFICATIONS: Develops ensemble relevance vector machines for predicting chlorophyll
#####################################################################################################################################
...
perf <- foreach(i=1:nrow(predmat), .combine=rbind) %dopar%{
  ...
  ##Train rvm
  chlrvm <- rvm(chl~ Rrs_412 + Rrs_443 + Rrs_469 + Rrs_488 + Rrs_531 + Rrs_547
                +Rrs_555 + Rrs_645 + Rrs_667+Rrs_678,
                data = StdLogCBtrain,
                kernel="rbfdot")
  
  ##Save the model 
  save(chlrvm, file = paste0("D:/Publication/CHL/RVM models/rvm",i,".RData"))
  
  ##Adjoin the information given
  c(chlrvm@nRV, ##Number of relevance vectors
    chlrvm@error,  ##training error
    as.vector(10^(
      predict(chlrvm, StdLogCBtest)*log_scale[1]+log_center[1]))) ##predicted on test dataset
}
stopImplicitCluster()
...
##Some basic metrics
n_alpha <- as.vector(perf[,1])
train_error <- as.vector(perf[,2])
