##################################################################################################################################
## PROGRAM: CHL ANN cv with parallel.R
## SPECIFICATIONS: Develops ensemble Artificial Neural Network for predicting chlorophyll
#####################################################################################################################################
rm(list=ls())
#####################################################################################################################################
## Libraries
...
perf <- foreach(i=1:nrow(modCBnew), .combine=rbind) %dopar%{
  require(neuralnet)##Need to boot library into every processor
  
  ##Remote single datapoint
  ...
  ##Train ann
  chlnet <- neuralnet(chl ~ Rrs_412 + Rrs_443 + Rrs_469 + Rrs_488 + 
                        Rrs_531 + Rrs_547 + Rrs_555 + Rrs_645 + Rrs_667 + Rrs_678, 
                      data = StdLogCBtrain, 
                      hidden = c(30, 20, 10), ##hidden layers
                      threshold = 0.5, #threshold to stop training
                      stepmax = 50000, #max number of steps
                      rep = 2, #if it fails to converge, this is backup
                      likelihood = TRUE)#so we can get aic, bic
  ##Save the model 
  save(chlnet, file = paste0("D:/Publication/CHL/ANN models/ann",i,".RData"))
  
  ##Adjoin the information given
  c(chlnet$result.matrix["aic",], ##Akaike Information Criterion
    chlnet$result.matrix["bic",], ##Bayesian Information Criterion
    as.vector(10^(compute(chlnet,
                                     StdLogCBtest[,-which(colnames(LogCBtest)=="chl")])
                             $net.result*log_scale[1]+log_center[1]))) ##predicted on test dataset
}
stopImplicitCluster()
...
predmat <- as.matrix(perf[,-c(1,2,3,4)])
errormat <- create_error_mat(predmat, CBtest$chl)
pred <- colMeans_trim(predmat, t)

##Some basic metrics
aic <- as.vector(perf[,c(1,2)])
bic <- as.vector(perf[,c(3,4)])
