...
plotMC <- function(MCraster, MClabel, CHLmin, CHLmax){
  ...
  image(MCraster, col=viridis(50), zlim=c(CHLmin,CHLmax), 
        axes=F, xlab="", ylab="", add=T)
  text(x=-75.8, y=38.75, labels=MClabel, cex=1.2, col="white", srt=90)
}
################################################################################
##For each of the machine learning algorihtms
for(m in 1:4){
  ##Read in the file
  myfile <- paste0("D:/Publication/CHL_",pattern[m],"_mc.tiff")
  ...
  for(i in 1:12){
    ##Find predicitons and plot them (already created)
    pred <- raster(paste0("D:/Publication/CHL_",mon[i],".grd"), band = m)
    plotMC(pred, mymonths[i], CHLmin, CHLmax)
  }
  ##Color scale
