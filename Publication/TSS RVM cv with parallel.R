##################################################################################################################################
## PROGRAM: TSS RVM cv with parallel.R
## SPECIFICATIONS: Develops ensemble RVM
## for predicting TSS
#####################################################################################################################################
...
perf <- foreach(i=1:nrow(predmat), .combine=rbind) %dopar%{
  ...
  ##Train rvm
  TSSrvm <- rvm(TSS~ Rrs_412 + Rrs_443 + Rrs_469 + Rrs_488 + Rrs_531 + Rrs_547
                +Rrs_555 + Rrs_645 + Rrs_667+Rrs_678,
                data = StdLogCBtrain,
                kernel="rbfdot")
  
  ##Save the model 
  save(TSSrvm, file = paste0("D:/Publication/TSS/RVM models/rvm",i,".RData"))
  
  ##Adjoin the information given
  c(TSSrvm@nRV, ##Number of relevance vectors
    TSSrvm@error,  ##training error
    as.vector(inverse_square(
      predict(TSSrvm, StdLogCBtest)*log_scale[1]+log_center[1]))) ##predicted on test dataset
}
stopImplicitCluster()
