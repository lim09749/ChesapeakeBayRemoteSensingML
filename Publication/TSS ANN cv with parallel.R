##################################################################################################################################
## PROGRAM: TSS ANN cv with parallel.R
## SPECIFICATIONS: Develops ensemble ANN
## for predicting TSS
#####################################################################################################################################
...
perf <- foreach(i=1:nrow(modCBnew), .combine=rbind) %dopar%{
  ...
  ##Train ann
  TSSnet <- neuralnet(TSS ~ Rrs_412 + Rrs_443 + Rrs_469 + Rrs_488 + 
  ...
  ##Save the model 
  save(TSSnet, file = paste0("D:/Publication/TSS/ANN models/ann",i,".RData"))
  
  ##Adjoin the information given
  c(TSSnet$result.matrix["aic",], ##Akaike Information Criterion
    TSSnet$result.matrix["bic",], ##Bayesian Information Criterion
    as.vector(inverse_square(compute(TSSnet,
                      StdLogCBtest[,-which(colnames(LogCBtest)=="TSS")])
